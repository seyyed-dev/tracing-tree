    ...
    FT: FormatTime,
{
    ...
    pub fn with_span_close_duration(self, enabled: bool) -> Self {
        Self {
            config: self.config.with_span_close_duration(enabled),
            ..self
        }
    }
        ...
        S: Subscriber + for<'span> LookupSpan<'span>,
    {
        ...
        let close_duration = self.format_span_close_duration(data, style);
        ...
        let should_write = match style {
            ...
            SpanMode::Close { verbose } => verbose || close_duration.is_some(),
        ...
        if should_write {
            ...
            if let Some(duration) = close_duration.as_deref() {
    ...
    fn format_span_close_duration(&self, data: &Data, style: SpanMode) -> Option<String> {
        if !self.config.span_close_duration || !matches!(style, SpanMode::Close { .. }) {
            return None;
        }
